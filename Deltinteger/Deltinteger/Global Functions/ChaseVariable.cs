    ...
    partial class GlobalFunctions
    {
        public static FuncMethod ChaseVariableAtRate(DeltinScript deltinScript) => new FuncMethodBuilder() {
            ...
            Parameters = new CodeParameter[] {
                new VariableParameter("variable", "The variable to manipulate. Player variables will chase the event player's variable. Must be a variable defined on the rule level.", VariableType.Dynamic, deltinScript.Types, new VariableResolveOptions() { CanBeIndexed = false, FullVariable = true }),
        ...
        public static FuncMethod StopChasingVariable(DeltinScript deltinScript) => new FuncMethodBuilder() {
            ...
            Parameters = new CodeParameter[] {
                new VariableParameter("variable", "The variable to stop. Must be a variable defined on the rule level.", VariableType.Dynamic, deltinScript.Types, new VariableResolveOptions() { CanBeIndexed = false, FullVariable = true })
            },
            Action = (actionSet, methodCall) => {
                VariableElements elements = ((VariableResolve)methodCall.AdditionalParameterData[0]).ParseElements(actionSet);
                WorkshopVariable variable = elements.IndexReference.WorkshopVariable;
