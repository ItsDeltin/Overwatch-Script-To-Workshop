...
namespace Deltin.Deltinteger.Elements
{
    public static class OptimizeElements
    {
        ...
        public static readonly Dictionary<string, Func<Element, Element>> Optimizations = new Dictionary<string, Func<Element, Element>>() {
            ...
            {"First Of", element => {
                if (((Element)element.ParameterValues[0]).Function.Name == "Array")
                {
                    var array = (Element)element.ParameterValues[0];

                    if (array.ParameterValues.Length > 0)
                        return (Element)array.ParameterValues.First();
                    else
                        return Element.Null();
                }
                return element;
            ...
            {"Value In Array", element => {
                ...
                if (element.ParameterValues[1] is NumberElement num)
                {
                    ...
                    // Value in array
                    if (array.Function.Name == "Array")
                    {
                        int index = (int)Math.Round(num.Value, MidpointRounding.AwayFromZero);
                        if (index >= 0 && index < array.ParameterValues.Length)
                            return (Element)array.ParameterValues[index];
                    }
