...
namespace Deltin.Deltinteger.Pathfinder
{
    public class ResolveInfoComponent : IComponent
    {
        ...
        public Element NextSegmentAttribute(Element player) => Element.TernaryConditional(
            Element.And(IsPathfinding(player), Element.Compare(Current.GetVariable(player), Operator.NotEqual, new NumberElement(-1))),
        ...
        public Element IsTravelingToNode(ActionSet actionSet, Element targetPlayer, Element node)
        {
            ...
            // Get the path.
            actionSet.AddAction(Element.While(Element.And(Element.Compare(
                look.GetVariable(),
            ...
            actionSet.AddAction(result.SetVariable(Element.Compare(look.Get(), Operator.Equal, node)));
        ...
        public Element IsTravelingToSegment(ActionSet actionSet, Element targetPlayer, Element segment)
        {
            ...
            actionSet.AddAction(result.SetVariable(Element.Compare(
                segment,
                Operator.Equal,
                Element.FirstOf(PathmapInstance.SegmentsFromNodes(PathmapReference.Get(targetPlayer), look.Get(), ParentArray.Get(targetPlayer)[look.Get()] - 1))
            )));
        ...
        public Element IsTravelingToAttribute(ActionSet actionSet, Element targetPlayer, Element attribute)
        {
            ...
            actionSet.AddAction(result.SetVariable(Element.Compare(attribute, Operator.Equal, AttributeArray.Get(targetPlayer)[look.Get()])));
